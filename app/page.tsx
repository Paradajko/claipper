    ...
    <main className="relative min-h-screen overflow-hidden">
      ...
      <header className="mx-auto flex max-w-7xl items-center justify-between px-4 py-5 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-3">
          <span className="flex h-10 w-10 items-center justify-center rounded-md bg-cyan-300 text-slate-950 shadow-[0_0_30px_rgba(34,211,238,.45)]">
            <Workflow size={22} />
          </span>
        ...
        <nav className="hidden items-center gap-6 text-sm text-slate-300 sm:flex">
          ...
          <Link href="/dashboard" className="rounded-md border border-cyan-300/30 px-4 py-2 text-cyan-100 hover:bg-cyan-300/10">Dashboard</Link>
        </nav>
      </header>
      ...
      <section className="mx-auto grid min-h-[calc(100vh-82px)] max-w-7xl items-center gap-10 px-4 pb-12 pt-8 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
        <div className="max-w-2xl">
          <Badge className="mb-5 border-cyan-300/30 bg-cyan-300/10 text-cyan-100">Clipping production workspace</Badge>
          ...
          <div className="mt-8 grid max-w-xl grid-cols-3 gap-3 text-sm text-slate-300">
            {["momenty", "hooky", "report výkonu"].map((item) => (
              <div key={item} className="rounded-md border border-white/10 bg-white/[0.04] px-3 py-3">
                <BadgeCheck className="mb-2 text-cyan-200" size={17} />
                {item}
              </div>
            ))}
          </div>
        ...
        <div className="relative">
          <div className="absolute -inset-4 rounded-[32px] bg-cyan-300/10 blur-3xl" />
          <Image
            ...
            height={1024}
            priority
            className="relative rounded-lg border border-cyan-200/20 shadow-glow"
          />
        </div>
      </section>
      ...
      <section id="mylaura" className="border-y border-white/10 bg-white/[0.025]">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 py-20 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
          <div>
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.22em] text-cyan-200">Rozdelenie rolí</p>
            <h2 className="text-4xl font-semibold tracking-tight text-white">MyLaura spúšťa kampane. Claipper vyrába obsah.</h2>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <Clapperboard className="mb-4 text-cyan-200" />
      ...
      <section id="ako-to-funguje" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="mb-10 max-w-2xl">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.22em] text-cyan-200">Workflow</p>
